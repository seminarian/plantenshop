...
$(function(){
	...
	$('<a href="#about" title="terug nr boven">terug nr boven</a>').insertBefore(':header:gt(1)').button( 
		{ icons: { secondary:'ui-icon-circle-triangle-n' } });

	var lijst = ['roger', 'evelyn', 'hilde', 'jan'];
	// $('<ul>').insertAfter('#team');
	// $('#team').after('<ul>').append('<li>eerst item</li>').append('<li>tweede item</li>');

	// $('<ul>').insertAfter('#team').append('<li>eerste item</li>').append('<li>tweede item</li>');
...
var checkNode = function(node) {
	//controleer of deze node in aanmerking komt voor de inhoudsopgave
